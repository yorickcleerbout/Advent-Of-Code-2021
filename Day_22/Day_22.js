...
function solve(input, part) {
    ...
    return cuboids.map(({ on, cuboid }) => (on ? 1 : -1) * cuboid.map(([min, max]) => max + 1 - min).reduce((acc, n) => acc * n)).reduce((acc, n) => acc + n);

}


console.log("How many cubes are on?", solve(input, 1));
console.log("How many cubes are on?", solve(input, 2));